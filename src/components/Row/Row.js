     ...
     function displayMovies() {
          return movies.map(movie => {
               if(movie.backdrop_path) {
                    return <img key={movie.id} src={`https://image.tmdb.org/t/p/original/${props.posterPath ? movie.poster_path : movie.backdrop_path}`} className={`row__poster ${props.posterPath ? "row__poster--large" : "" }`} alt={movie.title} onClick={() => { handleClick(movie) }} />
               }
